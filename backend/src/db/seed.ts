...
// Datos de referencia: 01-PROJECT_SPEC.md §5 y SDD-09-TESTS.md §0.2.
// Nunca se hardcodea una contraseña real/predecible (lección de
// AUDIT_GOVIND.md / AUDIT_MUZY.md, ver SDD-11 §9 #4 y #5): si no se provee
// por variable de entorno, se genera una aleatoria y se imprime una sola vez.

function randomPassword(): string {
  return crypto.randomBytes(9).toString("base64url");
}

async function seedUser(name: string, email: string, role: "admin" | "operario" | "dueño", envVar: string) {
  const password = process.env[envVar] && process.env[envVar]!.length >= 8 ? process.env[envVar]! : randomPassword();
  ...
  if (user) {
    console.log(`Usuario creado: ${email} (rol: ${role}) — contraseña: ${password}`);
  } else {
    console.log(`Usuario ya existía, se omite: ${email}`);
  }
}
...
async function main() {
  ...
  // Nota: suppliers/ingredients no tienen constraint UNIQUE a nivel de BD
  // (la unicidad de nombre de ingrediente se valida en la capa de aplicación,
  // ver SDD-06 ING-002), por lo que este script está pensado para correr
  // una sola vez sobre una base de datos limpia recién migrada.

  await seedUser("Ana Quispe", "admin@panaderia.bo", "admin", "SEED_ADMIN_PASSWORD");
  await seedUser("Carlos Mamani", "operario@panaderia.bo", "operario", "SEED_OPERARIO_PASSWORD");
  await seedUser("Beatriz Rojas", "dueño@panaderia.bo", "dueño", "SEED_DUENO_PASSWORD");
  ...
  for (const name of ingredientCategoryNames) {
    const [cat] = await db
      .insert(categories)
      .values({ name, kind: "ingrediente" })
      .onConflictDoNothing()
      .returning();
    if (cat) ingredientCategoryIds.set(name, cat.id);
    console.log(cat ? `Categoría de ingrediente creada: ${name}` : `Categoría ya existía, se omite: ${name}`);
  }
  ...
  for (const name of recipeCategoryNames) {
    const [cat] = await db
      .insert(categories)
      .values({ name, kind: "receta" })
      .onConflictDoNothing()
      .returning();
    if (cat) recipeCategoryIds.set(name, cat.id);
    console.log(cat ? `Categoría de receta creada: ${name}` : `Categoría ya existía, se omite: ${name}`);
  }

  // --- Proveedores ---
  const supplierIds: string[] = [];
  for (const supplier of seedSuppliers) {
    const [row] = await db.insert(suppliers).values(supplier).onConflictDoNothing().returning();
    if (row) supplierIds.push(row.id);
    console.log(row ? `Proveedor creado: ${supplier.name}` : `Proveedor ya existía, se omite: ${supplier.name}`);
  }

  // --- Ingredientes (con su precio inicial vía price history, nunca un campo suelto) ---
  const ingredientIds = new Map<string, string>();
  for (const seedIngredient of seedIngredientsData) {
    const [ingredient] = await db
      .insert(ingredients)
      .values({
        name: seedIngredient.name,
        baseUnit: seedIngredient.baseUnit,
        currentStock: seedIngredient.currentStock,
        minStock: seedIngredient.minStock,
        categoryId: ingredientCategoryIds.get(seedIngredient.category) ?? null,
        supplierId: supplierIds[seedIngredient.supplierIndex] ?? null,
        imageUrl: seedIngredient.imageUrl ?? null,
      })
      .onConflictDoNothing()
      .returning();
    ...
    if (ingredient) {
      await db.insert(ingredientPriceHistory).values({
        ...
        pricePerBaseUnit: seedIngredient.pricePerBaseUnit,
        effectiveAt: new Date(),
      });
      ingredientIds.set(seedIngredient.name, ingredient.id);
      console.log(`Ingrediente creado: ${seedIngredient.name}`);
    } else {
      console.log(`Ingrediente ya existía, se omite: ${seedIngredient.name}`);
    }
  }

  // --- Recetas (en orden: "Masa Base para Pan" antes de quien la anida) ---
  const recipeIds = new Map<string, string>();
  for (const seedRecipe of seedRecipesData) {
    const [recipe] = await db
      .insert(recipes)
      .values({
        name: seedRecipe.name,
        categoryId: recipeCategoryIds.get(seedRecipe.category) ?? null,
        description: seedRecipe.description,
        yieldQuantity: seedRecipe.yieldQuantity,
        yieldUnit: seedRecipe.yieldUnit,
        wastePercent: seedRecipe.wastePercent,
        prepTimeMinutes: seedRecipe.prepTimeMinutes,
        bakeTimeMinutes: seedRecipe.bakeTimeMinutes,
        imageUrl: seedRecipe.imageUrl ?? null,
      })
      .onConflictDoNothing()
      .returning();

    if (!recipe) {
      console.log(`Receta ya existía, se omite: ${seedRecipe.name}`);
      continue;
    }
    recipeIds.set(seedRecipe.name, recipe.id);

    for (const line of seedRecipe.lines) {
      if ("ingredient" in line) {
        const ingredientId = ingredientIds.get(line.ingredient);
        if (!ingredientId) {
          throw new Error(`Ingrediente no sembrado aún: ${line.ingredient} (requerido por receta ${seedRecipe.name})`);
        }
        await db.insert(recipeIngredients).values({
          recipeId: recipe.id,
          ingredientId,
          quantity: line.quantity,
          unit: line.unit as (typeof recipeIngredients.$inferInsert)["unit"],
        });
      } else {
        const subRecipeId = recipeIds.get(line.subRecipe);
        if (!subRecipeId) {
          throw new Error(`Sub-receta no sembrada aún: ${line.subRecipe} (requerida por receta ${seedRecipe.name})`);
        }
        await db.insert(recipeIngredients).values({
          recipeId: recipe.id,
          subRecipeId,
    ...
    console.log(`Receta creada: ${seedRecipe.name} (${seedRecipe.lines.length} líneas)`);
  }

  // --- Productos: uno por cada receta terminada (no las que son solo base) ---
  for (const seedRecipe of seedRecipesData) {
    if (!seedRecipe.productSalePrice) continue;
    const recipeId = recipeIds.get(seedRecipe.name);
    ...
    const [product] = await db
      ...
      .values({
        recipeId,
        name: seedRecipe.name,
        salePrice: seedRecipe.productSalePrice,
      })
      .onConflictDoNothing()
      .returning();
    ...
    console.log(
      product
        ? `Producto creado: ${seedRecipe.name} — precio venta Bs ${seedRecipe.productSalePrice}`
        : `Producto ya existía, se omite: ${seedRecipe.name}`,
    );
  }
